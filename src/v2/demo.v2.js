...
function handleFileInputChange(evt) {
    ...
    if (evt.srcElement.files.length > 0) {
        ...
        LocalFileReader.read(evt.srcElement.files, readMode)
            .then(onReadComplete);
    }
}
...
function InitializeDropdown() {
    ...
    if (readModeDropdown) {
        ...
        for (mode in LocalFileReader.ReadModes) {
            newOption = document.createElement("option");
            newOption.value = LocalFileReader.ReadModes[mode];
            newOption.innerHTML = mode.replace(/([a-z])([A-Z])/g, '$1 $2').trim();

            readModeDropdown.appendChild(newOption);
            if (LocalFileReader.ReadModes[mode] === LocalFileReader.ReadModes.Text) {
